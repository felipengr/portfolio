...
const ProjectCard: React.FC<ProjectCardProps> = ({ repoName, imageSrc, altText, isLoading = false }) => {
    const [projectData, setProjectData] = useState<{ name: string; html_url: string } | null>(null);
    ...
    useEffect(() => {
        const fetchRepoData = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Erro ao buscar dados do repositório: ${response.status}`);
                }
    ...
    return (
        <a href={projectData.html_url} target="_blank" rel="noopener noreferrer">
            ...
            >
                ...
                <div>
                    <p className="font-bold text-black dark:text-white mt-3">{projectData.name}</p>
                </div>
            </div>
        </a>
    );
