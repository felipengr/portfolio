...
export async function POST(req: Request) {
    try {
        ...
        try {
            ...
            return NextResponse.json({ error: "Erro ao verificar o reCAPTCHA (problema de rede)" }, { status: 500 });
        }


        const RECAPTCHA_SCORE_THRESHOLD = 0.5; // Ajuste este valor se necessário

        if (!recaptchaData.success || (recaptchaData.score !== undefined && recaptchaData.score < RECAPTCHA_SCORE_THRESHOLD) || recaptchaData.action !== 'submit') {
